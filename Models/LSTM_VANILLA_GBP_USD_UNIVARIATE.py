...
data = pd.read_csv("GBP_USD Historical Data.csv")
...
for k in range(5):
    ...
    fig1 = plt.figure(1)
    fig1.suptitle('Training and validation history LSTM TANH GBP-USD UNIVARIATE ' + str(look_back) +
    ...
    fig2 = plt.figure(2)
    fig2.suptitle('Predicted vs Actual LSTM TANH GBP-USD UNIVARIATE ' + str(look_back) + ' PAST OBSERVATIONS RUN ' +
    ...
    model.save_weights('lstm_vanilla_tanh_gbp_usd_univariate_' + str(look_back) + '_past_observations_run_' + str(k+1) +
                       '.h5')
