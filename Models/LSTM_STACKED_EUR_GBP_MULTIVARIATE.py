...
for k in range(5):
    model = Sequential()
    model.add(LSTM(100, activation='relu', input_shape=(look_back, features), return_sequences=True))
    ...
    fig1 = plt.figure(1)
    fig1.suptitle('Training and validation history LSTM STACKED EUR-GBP MULTIVARIATE GOLD OIL ' + str(look_back) +
    ...
    model.save_weights('lstm_stacked_eur_gbp_multivariate_gold_oil_' + str(look_back) + '_past_observations_run_' +
                       str(k+1) + '.h5')
