...
for k in range(5):
    ...
    fig1 = plt.figure(1)
    fig1.suptitle('Training and validation history LSTM BIDIRECTIONAL EUR-GBP UNIVARIATE ' + str(look_back) +
    ...
    fig2 = plt.figure(2)
    fig2.suptitle('Predicted vs Actual LSTM BIDIRECTIONAL EUR-GBP UNIVARIATE ' + str(look_back) +
                  ' PAST OBSERVATIONS RUN ' + str(k + 1), fontsize=8, y=0.99, verticalalignment='top')
    ...
    model.save_weights('lstm_bidirectional_eur_gbp_univariate_' + str(look_back) + '_past_observations_run_' +
                       str(k + 1) + '.h5')
