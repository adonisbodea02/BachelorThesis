...
training_data_X = training_data_X.reshape(training_data_X.shape[0], sub_sequence, 1, int(look_back/sub_sequence),
                                          features)
validation_data_X = validation_data_X.reshape(validation_data_X.shape[0], sub_sequence, 1, int(look_back/sub_sequence),
                                              features)
test_data_X = test_data_X.reshape(test_data_X.shape[0], sub_sequence, 1, int(look_back/sub_sequence), features)
...
for k in range(5):
    model = Sequential()
    model.add(ConvLSTM2D(filters=100, kernel_size=(1, 2), activation='relu', input_shape=(sub_sequence, 1,
                                                                                          int(look_back/sub_sequence),
                                                                                          features)))
    model.add(Flatten())
    model.add(Dense(1))
    model.compile(optimizer='adam', loss='mse', metrics=[metrics.mae, metrics.RootMeanSquaredError()])

    train_history = model.fit(training_data_X, training_data_Y, epochs=500, verbose=2,
    ...
    fig1 = plt.figure(1)
    fig1.suptitle('Training and validation history ConvLSTM EUR-GBP MULTIVARIATE GOLD OIL ' + str(look_back) +
    ...
    for i in test_data_X:
        sample = i.reshape((1, sub_sequence, 1, int(look_back/sub_sequence), features))
        predictions.append(model.predict(sample, verbose=0))
        predictions[-1] = predictions[-1][0]
    predictions = array(predictions)
    fig2 = plt.figure(2)
    fig2.suptitle('Predicted vs Actual ConvLSTM EUR-GBP MULTIVARIATE GOLD OIL ' + str(look_back) +
                  ' PAST OBSERVATIONS RUN '
    ...
    model.save_weights('convLSTM_eur_gbp_multivariate_gold_oil_' + str(look_back) + '_past_observations_run_' +
                       str(k + 1) + '.h5')
