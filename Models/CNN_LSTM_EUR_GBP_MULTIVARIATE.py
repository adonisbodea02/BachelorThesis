...
training_data_X = training_data_X.reshape(training_data_X.shape[0], sub_sequence, int(look_back/sub_sequence), features)
validation_data_X = validation_data_X.reshape(validation_data_X.shape[0], sub_sequence, int(look_back/sub_sequence),
                                              features)
test_data_X = test_data_X.reshape(test_data_X.shape[0], sub_sequence, int(look_back/sub_sequence), features)
...
for _ in range(5):
    model = Sequential()
    model.add(TimeDistributed(Conv1D(filters=100, kernel_size=2, activation='relu', input_shape=(look_back, features))))
    ...
    fig1 = plt.figure(1)
    fig1.suptitle('Training and validation history CNN-LSTM EUR-GBP MULTIVARIATE GOLD OIL ' + str(look_back) +
    ...
    for i in test_data_X:
        sample = i.reshape((1, sub_sequence, int(look_back/sub_sequence), features))
        predictions.append(model.predict(sample, verbose=0))
        predictions[-1] = predictions[-1][0]
    predictions = array(predictions)
    fig2 = plt.figure(2)
    fig2.suptitle('Predicted vs Actual CNN-LSTM EUR-GBP MULTIVARIATE GOLD OIL ' + str(look_back) + ' PAST OBSERVATIONS',
                  fontsize=8, y=0.99,
