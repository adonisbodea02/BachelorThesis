...
for k in range(5):
    model = Sequential()
    model.add(Dense(1000, activation='relu', input_dim=look_back))
    ...
    fig1 = plt.figure(1)
    fig1.suptitle('Training and validation history FFNN STACKED EUR-GBP UNIVARIATE ' + str(look_back) +
    ...
    fig2 = plt.figure(2)
    fig2.suptitle('Predicted vs Actual FFNN STACKED EUR-GBP UNIVARIATE ' + str(look_back) + ' PAST OBSERVATIONS RUN ' +
    ...
    model.save_weights('ffnn_stacked_eur_gbp_univariate_' + str(look_back) + '_past_observations_run_' + str(k+1) +
                       '.h5')
