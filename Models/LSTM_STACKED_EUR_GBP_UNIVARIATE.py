...
for k in range(5):
    model = Sequential()
    model.add(LSTM(100, activation='relu', input_shape=(look_back, 1), return_sequences=True))
    ...
    fig1 = plt.figure(1)
    fig1.suptitle('Training and validation history LSTM STACKED EUR-GBP UNIVARIATE ' + str(look_back) +
    ...
    fig2 = plt.figure(2)
    fig2.suptitle('Predicted vs Actual LSTM STACKED EUR-GBP STACKED ' + str(look_back) + ' PAST OBSERVATIONS RUN ' +
    ...
    model.save_weights('lstm_stacked_eur_gbp_univariate_' + str(look_back) + '_past_observations_run_' + str(k+1) +
                       '.h5')
